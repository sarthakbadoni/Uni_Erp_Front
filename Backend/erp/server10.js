...
// Utility for last room
async function getLastRoomNo(hostelId) {
  const res = await dynamo.query({
    TableName: 'HostelAssigned',
    KeyConditionExpression: 'HostelID = :h',
    ExpressionAttributeValues: { ':h': hostelId },
    ScanIndexForward: false,
    Limit: 1
  }).promise();
  if (res.Items.length > 0) {
    return parseInt(res.Items[0].RoomNo, 10) || 0;
  }
  return 0;
}
...
app.post('/students', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to add student or assign hostel.' });
  }
});
